...
CONFIG = PromotionConfig(
    ...
    cover_candidate_queue_priority=90,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Promote JPC staged offers via centrale USF promotion."
    )
...
def run_preview(limit: int) -> int:
    ...
    print(
        f"[PROMOTE] shop={CONFIG.shop_id} queued={len(rows)} write=False",
        flush=True,
    )
    ...
    for index, row in enumerate(rows, start=1):
        ...
        print(
            "[PROMOTE-PREVIEW]",
            {
                ...
                "shop_country": record.shop_country,
