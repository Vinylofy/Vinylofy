...
def parse_listing_page(
    ...
    *,
    ...
    anchors = soup.select('a[href*="/detail/"]')
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Refresh Sounds listing prices into prices using collection pages."
    )
    ...
    parser.add_argument(
        "--fast-price-sync",
        action="store_true",
        help="Gebruik snelle bulk price sync vanuit shop_product_links; sla generieke per-offer matching over.",
    )
...
def main() -> int:
    ...
    if args.write and not all_offers:
        raise SystemExit("[ERROR] Sounds listing refresh vond geen prijzen; schrijf niets weg.")
