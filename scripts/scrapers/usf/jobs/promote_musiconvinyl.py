...
CONFIG = PromotionConfig(
    ...
    cover_candidate_queue_priority=85,
    # Music On Vinyl wordt exact op EAN/barcode gematcht.
    # Geen artist/title fallback; canonieke metadata blijft extern leidend.
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Promote Music On Vinyl staged offers via centrale USF promotion."
    )
...
def run_preview(limit: int) -> int:
    rows = fetch_staged_rows(shop_id=CONFIG.shop_id, limit=limit)
    valid = 0
    invalid = 0
    print(f"[PROMOTE] shop={CONFIG.shop_id} queued={len(rows)} write=False", flush=True)
    for index, row in enumerate(rows, start=1):
        staged_offer_id = str(row["staged_offer_id"])
        try:
            record = staged_row_to_record(row=row, config=CONFIG, line_number=index)
        except Exception as exc:
            invalid += 1
            print(
                "[PROMOTE-PREVIEW-ERROR]",
                {"staged_offer_id": staged_offer_id, "reason": str(exc)},
                flush=True,
            )
            continue
        valid += 1
        print(
            "[PROMOTE-PREVIEW]",
            {
                ...
                "cover_candidate_url": record.cover_candidate_url,
            },
            flush=True,
        )
    print(
        "[PROMOTE] dry-run complete",
        {"valid": valid, "invalid": invalid, "databasewrites": False},
        flush=True,
    )
    return 0
...
def run_write(limit: int) -> int:
    result = promote_staged_offers_atomically(config=CONFIG, limit=limit)
    print(
        "[PROMOTE] done",
        {
            "shop": CONFIG.shop_id,
    ...
    for item in result.items:
        print(
            "[PROMOTED]",
            {
                ...
                "cover_candidate_inserted": item.cover_candidate_inserted,
            },
            flush=True,
        )
    for failure in result.failures:
        print(
            "[PROMOTE-ERROR]",
            {"staged_offer_id": failure.staged_offer_id, "reason": failure.reason},
            flush=True,
        )
    return 0
