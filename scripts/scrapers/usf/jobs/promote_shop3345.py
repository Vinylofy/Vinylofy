...
def payload_bool(value: Any) -> bool:
    ...
    return clean_public_text(value).lower() in {
        "1",
        "true",
        "yes",
        "ja",
    ...
    with conn.cursor(row_factory=dict_row) as cur:
        cur.execute(
            ...
            where s.shop_id = %s
              ...
              and s.availability = 'in_stock'
              and coalesce(
                    r.payload->'listing_payload'->>'availability',
                    'unknown'
                  ) = 'in_stock'
              and coalesce(
                    r.payload->'listing_payload'->>'is_secondhand',
                    'false'
                  ) = 'false'
              and coalesce(
                    r.payload
...
def staged_row_to_record(row: dict[str, Any], line_number: int) -> CanonicalRecord:
    ...
    if payload_bool(listing_payload.get("is_secondhand")):
        raise ValueError(
            "secondhand listing is not eligible for public promotion"
        )

    listing_availability = clean_public_text(
        listing_payload.get("availability")
    ).lower()

    if listing_availability != "in_stock":
        raise ValueError(
            "listing availability is not in_stock"
        )

    if not payload_bool(
        listing_payload.get("listing_cta_add_to_cart")
    ):
        raise ValueError(
            "listing has no active Add to cart"
        )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Promote 3345 staged offers naar products/prices."
    )
...
def main() -> int:
    if "--write" in sys.argv[1:]:
        raise SystemExit(
            "[DISABLED] promote_shop3345.py mag geen 3345-voorraad meer schrijven. "
