#!/usr/bin/env python3
# FiftiesStore listing-first live price sync.
#
# Afgeleid van de bestaande iMusic-sync zodat dezelfde databaseconventies
# worden gebruikt. Deze job werkt uitsluitend reeds gevalideerde
# public.prices-rijen bij. Nieuwe offers worden hier niet gepubliceerd.
...
SHOP_NAME = "FiftiesStore"
SHOP_DOMAIN = "fiftiesstore.nl"
SHOP_COUNTRY = "NL"
CURRENCY = "EUR"
...
def source_url_for_price(link: dict[str, Any], payload: dict[str, Any]) -> str:
    return (
        ...
        or normalize_text(payload.get("product_url"))
        or normalize_text(payload.get("url"))
...
def fetch_listing_links(cur, *, limit: int) -> list[dict[str, Any]]:
    cur.execute(
        ...
        where shop_id = %s
          ...
          and coalesce(payload->>'listing_sale_price_raw', payload->>'sale_price', payload->>'current_price', payload->>'listing_price_raw', payload->>'listing_price_hint', payload->>'listing_price', payload->>'price_raw', payload->>'price') is not null
        ...
        with conn.cursor(row_factory=dict_row) as cur:
            ...
            for link in links:
                ...
                for ean_candidate in (
                    ...
                    ean = normalize_ean(ean_candidate)
                    if ean:
                        break
                if not ean:
                    stats.skipped_invalid_ean += 1
                    print(
                        "[FIFTIESSTORE-PRICE-SYNC-SKIP]",
                        {
                            "reason": "invalid_ean",
                ...
                price_raw = normalize_text(
                    payload.get("listing_sale_price_raw")
            or payload.get("sale_price")
            or payload.get("current_price")
            or payload.get("listing_price_raw")
            ...
            or payload.get("price")
                ...
                if not product_url:
                    product_url = f"https://www.fiftiesstore.nl/search?q={ean}"
                ...
                if existing_price is None:
                    # Belangrijk: nieuwe FiftiesStore offers mogen NIET via price-only live.
                    # Die moeten eerst door de bestaande FiftiesStore stage/promote-validatie.
                    stats.skipped_no_existing_price += 1
                    print(
                        "[FIFTIESSTORE-PRICE-SYNC-SKIP]",
                        {
                            "reason": "no_existing_fiftiesstore_price",
                            "ean": ean,
                            "product_id": product_id,
                            "price_raw": price_raw,
                            "product_url": product_url,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Sync FiftiesStore listing prices to existing validated public.prices rows. "
            "Nieuwe FiftiesStore offers worden bewust niet aangemaakt; die moeten eerst door detail/stage/promote."
        )
    )
...
def main() -> int:
    ...
    if stats.errors > 0:
        raise SystemExit(
            f"[FIFTIESSTORE-PRICE-SYNC-FAIL] "
            f"{stats.errors} fouten tijdens de sync."
        )

    if args.write and stats.candidates == 0:
        raise SystemExit(
            "[FIFTIESSTORE-PRICE-SYNC-FAIL] "
            "Geen recente FiftiesStore listinglinks gevonden."
        )

    if args.write and stats.processed == 0:
        raise SystemExit(
            "[FIFTIESSTORE-PRICE-SYNC-FAIL] "
            "Geen bestaande live FiftiesStore-prijzen ververst."
        )
