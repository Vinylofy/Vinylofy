...
def normalize_title(value: Any) -> str | None:
    ...
    text = re.sub(r"\s*(?:&ndash;|–|-|\|)\s*Variaworld\s*$", "", text, flags=re.IGNORECASE)
    text = text.lower()
...
def extract_title_from_html(html: str | None) -> str | None:
    ...
    for pattern in patterns:
        ...
        if match:
            ...
            if title:
                title = re.sub(r"\s*[|–-]\s*Variaworld\s*$", "", title, flags=re.IGNORECASE)
                return title.strip()
...
def extract_price_from_html(html: str | None) -> Decimal | None:
    """Variaworld policy: actuele prijs komt alleen uit listing/raw price_raw, nooit uit detail HTML."""
    return None
