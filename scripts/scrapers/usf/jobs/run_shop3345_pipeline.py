...
def run_step(
    ...
    print(
        f"\n[SHOP3345-PIPELINE] START {label}",
        flush=True,
    )
    print(
        "[SHOP3345-PIPELINE] CMD",
        " ".join(command),
        flush=True,
    )
    ...
    print(
        f"[SHOP3345-PIPELINE] DONE {label}",
        flush=True,
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run de zelfstandige 3345 USF-pipeline."
    )
...
def main() -> int:
    ...
    if not args.skip_listing:
        ...
        print(
            "[SHOP3345-PIPELINE] SKIP listing",
            flush=True,
        )
