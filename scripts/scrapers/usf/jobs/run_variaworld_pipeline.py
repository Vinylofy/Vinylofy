...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run Variaworld USF listing-first pipeline.")
    ...
    print("[PIPELINE] Variaworld USF pipeline", flush=True)
    print(f"[PIPELINE] listing_max_pages={args.listing_max_pages}", flush=True)
    print(f"[PIPELINE] detail_limit={args.detail_limit}", flush=True)
    print(f"[PIPELINE] stage_limit={args.stage_limit}", flush=True)
    print(f"[PIPELINE] promote_limit={args.promote_limit}", flush=True)
    print(f"[PIPELINE] quarantine_limit={args.quarantine_limit}", flush=True)
    print(f"[PIPELINE] write={args.write}", flush=True)
    print(f"[PIPELINE] fast_price_sync={args.fast_price_sync}", flush=True)

    if not args.skip_listing_refresh:
        listing_command = [
            sys.executable,
            "-m",
            "scripts.scrapers.usf.jobs.refresh_variaworld_listing_prices",
            "--start-page",
        ...
        run_step("refresh_variaworld_listing_prices", listing_command)
    ...
    if not args.skip_detail:
        run_step(
            ...
            [
                sys.executable,
                "-m",
                "scripts.scrapers.usf.jobs.detail_variaworld",
                "--limit",
    ...
    if not args.skip_stage:
        run_step(
            ...
            [
                sys.executable,
                "-m",
                "scripts.scrapers.usf.jobs.stage_variaworld",
                "--limit",
    ...
    if not args.skip_promote:
        promote_command = [
            sys.executable,
            "-m",
            "scripts.scrapers.usf.jobs.promote_variaworld",
            "--limit",
            str(args.promote_limit),
        ]
        if args.write:
            promote_command.append("--write")
        run_step("promote_variaworld", promote_command)

    if not args.skip_quarantine:
        quarantine_command = [
            sys.executable,
            "-m",
            "scripts.scrapers.usf.jobs.quarantine_variaworld",
            "--limit",
            str(args.quarantine_limit),
        ]
        if args.write:
            quarantine_command.append("--write")
        run_step("quarantine_variaworld", quarantine_command)
