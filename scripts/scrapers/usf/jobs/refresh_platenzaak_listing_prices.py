...
from scripts.scrapers.usf.jobs.discover_platenzaak import SHOP_ID, discover_links
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Refresh Platenzaak listing prices into prices using collection pages."
    )
    parser.add_argument(
        "--max-pages",
        type=int,
...
def main() -> int:
    ...
    for link in links:
        ...
        offers.append(
            ListingOffer(
                ...
                currency=clean(payload.get("currency")) or "EUR",
                seen_at=seen_at,
                raw=payload,
            )
        )
    ...
    if not links:
        raise SystemExit("[ERROR] Platenzaak listing refresh leverde geen links op.")

    if args.write and not offers:
        raise SystemExit("[ERROR] Platenzaak listing refresh vond geen prijzen; schrijf niets weg.")
