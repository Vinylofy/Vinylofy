...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Stage de nieuwste Bob's Vinyl raw snapshots "
            "naar staged_offers via de generieke USF-stagingcore."
        )
    )
    parser.add_argument(
        "--limit",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Schrijf staged offers. Zonder --write "
            "wordt alleen een dry-run uitgevoerd."
        ),
    )
    return parser
...
def main() -> int:
    ...
    if not args.write:
        print(
            "[STAGE] dry-run complete; geen databasewrites.",
            flush=True,
        )
