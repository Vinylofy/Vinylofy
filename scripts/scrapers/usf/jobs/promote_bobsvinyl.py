...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Promote Bob's Vinyl staged offers via de centrale "
    ...
    for index, row in enumerate(rows, start=1):
        ...
        print(
            "[PROMOTE-SAMPLE]",
            {
                ...
                "product_url": clean(
                    getattr(record, "product_url", None)
                ),
            },
            flush=True,
        )
...
def run_write(limit: int) -> int:
    ...
    for item in result_dict.get("items", [])[:5]:
        print(
            "[PROMOTE-WRITE]",
            {
                key: clean(value)
    ...
    for failure in result_dict.get("failures", [])[:5]:
        print(
            "[PROMOTE-FAILURE]",
            {
                key: clean(value)
