    ...
    with conn.cursor(row_factory=dict_row) as cur:
        cur.execute(
            ...
            where s.shop_id = %s
              ...
              and s.availability in ('in_stock', 'preorder')
              and coalesce(
...
def staged_row_to_record(row: dict[str, Any], line_number: int) -> CanonicalRecord:
    ...
    if listing_availability not in {
        "in_stock",
        "preorder",
    }:
        raise ValueError(
            "listing availability is not publishable"
        )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Promote CD Hal Ruinen staged offers naar products/prices."
    )
