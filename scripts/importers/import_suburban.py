...
def resolve_ean_rejection(row: dict) -> str:
    detail_status = normalize_text(row.get("detail_status")).casefold()
    if detail_status in {"", "pending", "listing"}:
        return "detail_not_attempted"
    if detail_status == "technical_error":
        return "detail_technical_error"
    return "missing_or_invalid_ean"


def map_suburban_row(row: dict, line_number: int) -> tuple[CanonicalRecord | None, str | None]:
...
SHOP_DEFINITION = ShopImporterDefinition(
    key="suburban",
    config=CONFIG,
    importer_module="scripts.importers.import_suburban",
    scraper_command_env="VINYLOFY_SCRAPER_CMD_SUBURBAN",
    storage_prefix="suburban",
    files=ImportFileLayout(
        csv_output_path="data/raw/suburban/suburban_master.csv",
        rejects_path="output/suburban_rejects.csv",
        summary_path="output/suburban_import_summary.json",
    ),
    row_mapper=map_suburban_row,
    description="Import Suburban listing-first master CSV into Supabase/Postgres",
    required_columns=(
        "scraped_at",
        "product_url",
        "ean",
    ...
    tags=("vinyl", "woocommerce", "listing-first", "barcode-gated"),
