-- Fase 8.1 — prevent terminal review jobs from being reclaimed.
--
-- Proven root cause:
-- claim_next_cover_job included review in the automatically claimable
-- statuses. A no_candidates job is completed as review, so the same queue
-- row could immediately be claimed again in the same worker run.
--
-- Contract after this migration:
-- - pending and retry_later are automatically claimable;
-- - review is terminal for the normal worker;
-- - candidate refresh may later reactivate the existing queue row by
--   setting it back to pending when new candidates are discovered.
...
begin
  ...
  with next_job as (
    ...
    where q.status in ('pending', 'retry_later')
      and (
