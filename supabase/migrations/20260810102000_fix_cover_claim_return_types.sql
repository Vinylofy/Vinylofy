-- Fix runtime row-type mismatch in claim_next_cover_job.
--
-- The RPC contract declares its textual result columns as text.
-- PostgreSQL PL/pgSQL RETURN QUERY requires the runtime row types to
-- match that declared table contract exactly; products.format_label
-- is currently varchar(14). Cast all textual outputs explicitly so
-- the function contract is deterministic.
